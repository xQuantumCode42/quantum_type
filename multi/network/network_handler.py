...
class NetworkHandler:
    def __init__(self, is_host, ip, port):
        ...
        print(f"NetworkHandler initialized as {'host' if is_host else 'client'} with IP {ip} and port {port}")
        ...
        if self.is_host:
            try:
                ...
                if self.game and hasattr(self.game, 'ui'):
                    self.game.ui.messagebox.showinfo("Port Changed", f"Using port {self.port} instead")
    ...
    def _send_raw_message_to_client(self, msg, client_socket):
        ...
        try:
            ...
            print(f"Sending direct message: {msg['type']}, size: {len(msg_bytes)} bytes")
                            ...
                            if len(header_data) == 4:
                                ...
                                if len(data) == msg_len:
                                    ...
                                    try:
                                        ...
                                        print(f"Host JSON decode error: {str(e)}, data length: {len(data)}")
                        ...
                        if len(header_data) == 4:
                            ...
                            if len(data) == msg_len:
                                ...
                                try:
                                    ...
                                    print(f"Client JSON decode error: {str(e)}, data length: {len(data)}")
                    except socket.error:
                        # No data available
                        time.sleep(0.1)
                except Exception as e:
    ...
    def connect(self, host_ip):
        try:
            ...
            try:
                ...
                if self.game and hasattr(self.game, 'ui'):
                    self.game.ui.messagebox.showerror("Connection Error", "Could not connect to host")
                self.running = False
        except Exception as e:
            print(f"Connection error: {str(e)}")
            if self.game and hasattr(self.game, 'ui'):
                self.game.ui.messagebox.showerror("Connection Error", f"Could not connect to host: {str(e)}")
            self.running = False
